...
CREATE CONSTRAINT decl_unique IF NOT EXISTS
  ...
  REQUIRE d.symbol IS UNIQUE;

CREATE CONSTRAINT symbol_unique IF NOT EXISTS
  FOR (s:Symbol) REQUIRE s.name IS UNIQUE;

// Indexes for lookup performance
CREATE INDEX fn_line IF NOT EXISTS
  FOR (f:CppFunctionDefinition) ON (f.lineNumber);

CREATE INDEX file_name IF NOT EXISTS
  FOR (f:SourceFile) ON (f.name);
\end{minted}
...
\begin{figure}[htbp]
  ...
  \caption{SysML v2 connections diagram: node-type hierarchy and property graph
    schema relationships between all 11 node labels.}
