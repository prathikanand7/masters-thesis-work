...
\begin{description}
  \item[\texttt{interaction}] A named container for a scenario-specific interaction
    model, analogous to a \ac{UML} interaction.
  \item[\texttt{send} / \texttt{receive}] Action semantics that model
    message transmission and reception between lifelines.
  \item[\texttt{MessageOccurrenceSpecification}] A timed event on a lifeline
    representing the sending or reception of a message.
  \item[\texttt{CombinedFragment}] Control structures such as \texttt{alt},
    \texttt{loop}, and \texttt{opt} that express conditional or iterative
    behaviour.
\end{description}

An example in \sysml textual notation, corresponding to the S1
scenario (Aggregator notification chain) used in this thesis:

\begin{minted}[fontsize=\footnotesize]{text}
package AggregatorNotification {
    part def SynchronizedRunner;
    part def Aggregator;
    part def CPSLogger;

    interaction S1_AggregatorChain {
        part runner : SynchronizedRunner;
        part agg    : Aggregator;
        part log    : CPSLogger;

        message m1 from runner to agg { :>> name = "getAll"; }
        message m2 from runner to log { :>> name = "stream"; }
        message m3 from runner to log { :>> name = "flush"; }
    }
}
\end{minted}
