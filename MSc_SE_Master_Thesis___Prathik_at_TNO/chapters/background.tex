...
Finally, it summarises how scenarios are scoped through graph queries.
...
Reconstructing a diagram for a single scenario requires narrowing a large
call graph down to the interactions that belong to that scenario. Rather
than adopt a formal program-slicing procedure over a program dependence
graph, this thesis performs scenario scoping directly as a declarative
graph query.

The scope of a scenario is expressed as a set of participating components
(for example, \texttt{Synchronization} calling into \texttt{Aggregation}
and \texttt{Logging}). A scenario-scoped Cypher query then retains only the
call edges whose caller and callee belong to those components. This keeps
the interactions that are relevant to the scenario and discards those that
belong to other, concurrently exercised behaviour.

The concrete query used in this thesis is described in
\cref{sec:combine-scope}. It operates over the static property graph
rather than over a source-level program representation, and the same
component-scope constraint is applied to the trace-derived sequence table.
