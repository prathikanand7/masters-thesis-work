...
scope. The two evidence sources are kept apart, tagged by origin, until an
\ac{LLM} performs agentic synthesis, judging which single-source signals
to keep and which to discard, rather than passing an unexamined union
straight through. The synthesised interactions are then projected into \sysml\ textual
notation.
...
\begin{enumerate}\setcounter{enumi}{3}
  ...
  \item A \textbf{set of twelve natural-language agent skills} that
    ...
    on any codebase without code changes; eleven follow a fixed rule set
    given their inputs, and one (\texttt{agentic-synthesis}) is
    deliberately specified as \ac{LLM} judgement guidance instead
    (\cref{sec:skill-design,sec:agentic-synthesis}).
