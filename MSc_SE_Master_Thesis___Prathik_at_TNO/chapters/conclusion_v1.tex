...
scenario's relevant components, producing both a static dependency table
and a reachable function set. Guided instrumentation then captures
\texttt{trace\_slice.txt} at the architecture-level call sites. A
Neo4j-based reachability filter (Phase~3, condition~C5) scopes the trace to
events whose caller is in the reachable function set, removing
setup-phase boilerplate and writing \texttt{interactions.json} and
\texttt{sequence.json}. The resulting interactions are then projected
