...
C4's two false positives on S1a and S1b are setup-phase events
(\texttt{Aggregator.add}~$\to$~\texttt{RAIILogStream.stream}) that fire
before the scenario entry point; their callers are not in the
Phase~1 reachable function set, so the reachability filter drops them
(\cref{sec:evidence-synthesis}), eliminating C4's remaining false
positives and yielding perfect scores.
