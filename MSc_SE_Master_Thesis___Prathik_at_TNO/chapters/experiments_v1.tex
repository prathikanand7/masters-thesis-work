...
For C5, the synthesis step applies a Neo4j-based reachability filter to
\texttt{trace\_slice.txt}: only events whose \texttt{ClientFunction} is
reachable from the scenario entry points via \texttt{CppCalls*0..8}
traversal are retained. This removes setup-phase boilerplate (e.g.\
\texttt{Aggregator.add} firing before any runner starts) that the
guided instrumentation sites fire unconditionally.
For C6, the same filter is applied to \texttt{full\_trace.txt}.
C4 reads \texttt{trace\_slice.txt} without the reachability filter.
C7--C9 pass C2, C3, and C4 candidate lists to a two-prompt \ac{LLM}
chain.

On the running edge
\texttt{SynchronizedRunner.runSynchronized}~$\to$~\texttt{Aggregator.getAll},
the guided trace (\texttt{trace\_slice.txt}) captures this interaction
because its probe site fires during the scenario-entry-point call.
For S1a, all 8 sequence events and both interaction edges in the ground
truth are present in \texttt{trace\_slice.txt}; for S1b, all 5 sequence
events and 3 interaction edges are captured (8 in S1a, 5 in S1b).
After the reachability filter (C5), setup-phase events are removed and
only scenario-logic events remain, yielding perfect F1 on both scenarios.
