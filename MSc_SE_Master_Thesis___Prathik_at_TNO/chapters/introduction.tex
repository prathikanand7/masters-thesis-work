...
The approach consists of five stages, implemented as a chain of
composable agent skills and tools (\cref{sec:skill-design}): a
preparatory static-analysis tool populates the graph once per codebase,
after which natural-language-invocable skills query, instrument and
render from it. First, \textbf{static extraction}: a call graph is
extracted from the target system's \Cpp\ source by \emph{Renaissance}, a
semantic-level \Cpp\ analyser (\cref{sec:static-analysis}), and stored in
a \neo \acl{GDB} using the schema in \cref{sec:pg-schema}; for pub-sub
systems routing messages through callback registration, such as
CPSCore's IDC layer, this stage additionally resolves the implicit
routing topology (\cref{sec:macro-resolution}). Second, \textbf{code
graph query} (the \texttt{text-to-cypher} skill): a scenario description
is translated into a Cypher query and executed against the static
property graph, retaining only the interactions whose caller and callee
belong to the scenario's declared components. Third,
...
combined by a deterministic set operation (intersection) when the
reconstructed models are compared during evaluation. Fifth, \textbf{SysML v2
