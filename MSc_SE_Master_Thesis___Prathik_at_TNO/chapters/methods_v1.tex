...
The four-phase pipeline is also exposed as twelve composable agent
skills invocable via natural-language prompt, providing a
practitioner-facing interactive interface for applying the pipeline to
a new codebase. The H1, H2, and H3 evaluations run the pipeline
through the underlying Python scripts directly, not through the skill
layer. The skill layer was chosen over a monolithic implementation for
three reasons:
...
and role in the pipeline. Deterministic skills (instrumentation,
synthesis, and diagram generation) follow a fixed rule set given their
inputs; query-generation skills (\texttt{text-to-cypher},
\texttt{cypher-to-output}, \texttt{get-cypher}) use an LLM and may
produce varied output for the same input.
