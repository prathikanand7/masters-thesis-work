...
\emph{Understand and Structure101}, which produce rich static dependency graphs but
...
More recently, \textcite{abdelmalek:survey} survey and categorise approaches
for reverse-engineering behavioural UML diagrams, grouping them into static,
dynamic and hybrid analysis, and themselves propose a hybrid approach that
uses static analysis to identify system entry points and guide dynamic
instrumentation. Neither their survey nor their proposed approach addresses
\Cpp\ or publish-subscribe indirection specifically, and their hybrid
technique still requires exercising the target behaviour at runtime rather
than reconstructing scenario-specific models from incomplete traces.
...
In particular, three things are missing from prior work. First, none combines
static call-graph evidence with runtime traces for \sysml interaction model
reconstruction while keeping track of which source each interaction came from.
Second, none applies scenario-scoped graph querying to focus the result on a
single scenario. Third, none targets \sysml as the output formalism.
