...
def extract_payload(result: object) -> Optional[object]:
    """
    Extract a JSON-able Python object or text from a FastMCP CallToolResult.
    Works across common return shapes:
      - JSON content items
      - structuredContent
      - text content items
    Returns: dict/list/str/None
    """
    # Prefer JSON items in content
    for item in getattr(result, "content", []) or []:
        if getattr(item, "type", None) == "json":
            return getattr(item, "json", None)

    # Structured content (some servers use this)
    sc = getattr(result, "structuredContent", None)
    if sc is not None:
        return sc

    # Fallback: concatenate text items
    texts = []
    for item in getattr(result, "content", []) or []:
...
def clean_cypher_query(cypher: str) -> str:
    """Remove backticks from a Cypher query for cleaner output.

    Args:
        cypher: The Cypher query string to clean.

    Returns:
        The cleaned Cypher query string with backticks removed.
    """
