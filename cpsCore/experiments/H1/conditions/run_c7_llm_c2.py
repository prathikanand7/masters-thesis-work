"""
run_c7_llm_c2.py — H1 Condition 7: LLM refined from static analysis (C2 candidates)
-------------------------------------------------------------------------------------
Two-call LLM chain per scenario:

  Call 1 — INTERACTIONS: given the C2 static call-graph candidates and the source
            code, decide which edges actually fire at runtime and add any that the
            static analysis missed (e.g. dynamically-dispatched calls).
...
The C2 candidate list acts as a prior: it focuses the LLM on statically-reachable
edges while allowing it to correct over-approximations (edges reachable in code but
not executed for this specific test) and under-approximations (dynamic dispatch).

Usage:
    python run_c7_llm_c2.py --scenario S1a
    python run_c7_llm_c2.py --all

Input:  conditions/C2_static_only/<scenario>/interactions.json   (static candidates)
        scenarios/<scenario>/source_snippet.txt
Output: conditions/C7_llm_c2/<scenario>/interactions.json
        conditions/C7_llm_c2/<scenario>/sequence.json
"""
import argparse
import json
import os
...
LABEL = "C7_llm_c2"
C2_LABEL = "C2_static_only"

# ── Prompt 1: interactions ─────────────────────────────────────────────────
PROMPT_INTERACTIONS = """\
You are a software architect analysing the CPSCore C++ framework.

The following cross-component interactions were identified by STATIC call-graph
analysis (Neo4j forward reachability from the test entry point) for this scenario.
Static analysis is sound but not precise — it includes every edge reachable through
the code but some edges may NOT actually fire during this specific test execution.

Static candidates:
{static_candidates}

Using the C++ source code below, determine the ACTUAL cross-component interactions
that execute during this test. You MUST:
  1. Keep candidates that genuinely fire during the test.
  2. Remove candidates that are statically reachable but do NOT fire for this test.
  3. Add any interactions you see in the source code that are missing from the list
     (e.g. dynamically-dispatched calls or calls the static graph missed).
...
def run_scenario(scenario: str) -> None:
    ...
    c2_path = interactions_path(C2_LABEL, scenario)
    if not snippet_path.exists():
        print(f"  [SKIP] {snippet_path} not found")
        return
    if not c2_path.exists():
        print(f"  [SKIP] C2 output not found: {c2_path} — run run_c2_static_only.py first")
        return

    source_code = snippet_path.read_text(encoding="utf-8")
    c2_data = json.loads(c2_path.read_text(encoding="utf-8"))

    # Format C2 candidates for the prompt
    static_candidates = "\n".join(
    ...
    # ── Call 1: interactions ────────────────────────────────────────────────
    print(f"  [Call 1] Refining static candidates ...")
    raw1 = _llm_call(client, PROMPT_INTERACTIONS.format(
        static_candidates=static_candidates, source_code=source_code))
    try:
    ...
    print(f"  C7 → {len(interaction_elements)} interactions, {len(seq_steps)} sequence steps")
