"""
run_c8_llm_c3.py — H1 Condition 8: LLM denoised from raw trace (C3 candidates)
---------------------------------------------------------------------------------
Two-call LLM chain per scenario:

  Call 1 — INTERACTIONS: given the C3 raw instrumentation candidates and the source
            code, filter out intra-component and setup events, keep genuine
            cross-component architectural interactions, and add any that were missed.
...
The C3 candidate list has perfect recall but poor precision: it includes
intra-component calls (source == target), test-fixture setup events, and every
cross-component call observed — including architectural noise. The LLM's task is
to denoise that list using its understanding of the source code.

Usage:
    python run_c8_llm_c3.py --scenario S1a
    python run_c8_llm_c3.py --all

Input:  conditions/C3_dynamic_only/<scenario>/interactions.json   (raw trace candidates)
        scenarios/<scenario>/source_snippet.txt
Output: conditions/C8_llm_c3/<scenario>/interactions.json
        conditions/C8_llm_c3/<scenario>/sequence.json
"""
import argparse
import json
import os
...
LABEL = "C8_llm_c3"
C3_LABEL = "C3_dynamic_only"

# ── Prompt 1: interactions ─────────────────────────────────────────────────
PROMPT_INTERACTIONS = """\
You are a software architect analysing the CPSCore C++ framework.

The following events were captured by RUNTIME INSTRUMENTATION of this test scenario
using a full, unfiltered execution trace. Because the trace is unfiltered, it contains:
  - Intra-component calls (source == target): these are INTERNAL implementation
    details within a single module, NOT architectural interactions.
  - Setup/teardown events fired by the test fixture before or after the scenario
    logic executes (e.g. object registration calls in the test body).
  - True cross-component architectural interactions between different modules.

Instrumented candidates (raw, unfiltered):
{trace_candidates}
...
architectural interactions for this test scenario. You MUST:
  1. Remove all intra-component events (source == target) — always excluded.
  2. Remove setup/teardown events that fire outside the scenario logic.
  3. Keep genuine cross-component interactions that are part of the scenario execution.
  4. Add any cross-component interactions you see in the source code that the
...
def run_scenario(scenario: str) -> None:
    ...
    c3_path = interactions_path(C3_LABEL, scenario)
    if not snippet_path.exists():
        print(f"  [SKIP] {snippet_path} not found")
        return
    if not c3_path.exists():
        print(f"  [SKIP] C3 output not found: {c3_path} — run run_c3_dynamic_only.py first")
        return

    source_code = snippet_path.read_text(encoding="utf-8")
    c3_data = json.loads(c3_path.read_text(encoding="utf-8"))

    # Format C3 candidates for the prompt — include all raw events (even intra-component)
    trace_candidates = "\n".join(
    ...
    # ── Call 1: interactions ────────────────────────────────────────────────
    print(f"  [Call 1] Denoising trace candidates ...")
    raw1 = _llm_call(client, PROMPT_INTERACTIONS.format(
        trace_candidates=trace_candidates, source_code=source_code))
    try:
    ...
    print(f"  C8 → {len(interaction_elements)} interactions, {len(seq_steps)} sequence steps")
