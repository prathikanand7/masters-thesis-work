    ...
    case RunStage::NORMAL:
        {
            ...
            {
                CPSLOG_DEBUG << "IPC subscribing on SIGINT via SignalHandler";
...
IPC::publishPackets(const std::string& id, const IPCOptions& options)
{
    ...
    {
        return {
            publishOnRedis(id, params.maxPacketSize()),
            [](const Packet& p)
            {
                return p;
            }
    ...
    {
        return {
            publishOnSharedMemory(id, params.maxPacketSize()),
            [](const Packet& p)
            {
                return p;
            }
...
IPC::sigintHandler(int sig)
{
    ...
    {
        CPSLOG_DEBUG << "IPC caught signal " << sig << ". Deleting subscriptions and publications.";
...
Subscription
IPC::subscribeOnPackets(const std::string& id, const std::function<void (const Packet&)>& slot,
